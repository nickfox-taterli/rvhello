...
RV32I 单核, 取指与访存共用一组 valid/ready 总线, 经地址译码器分发到四个从端. 乘法子集当前实现 MUL, MULH, MULHSU, MULHU, 每条乘法固定 32 拍迭代完成:

- 锁存两个操作数的 magnitude 和符号.
- 每拍按 multiplier[0] 做一次条件累加, 然后被乘数左移、乘数右移.
- 最后一拍对完整 64 位积恢复符号, 再写回低 32 位或高 32 位.
...
- `src/board/` 顶层与数码管扫描
- `fw/` 固件源码 (link.ld / start.S / main.c), `-march=rv32i -mabi=ilp32`; 乘法自检使用 `.insn` 保持编译器不会假定 DIV/REM 可用.
- `sim/` 仿真测试台
